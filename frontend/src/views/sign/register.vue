        ...
        <div class="user-auth__wrap">
            <div class="sign__wrap">
                ...
                >
                    ...
                    <p :class="['password-tip', { 'show-warning': form.showWarning }]">密码长度为 8 到 16 个字符且同时包含大写和小写字母, 数字或特殊字符</p>
                ...
                <p class="copyright">提交注册即代表同意我们的 [使用条款]...</p>
                <p class="copyright">@2019-2021 All Right Reserved · claude</p>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import { register } from '@js/common/services';

    export default {
        data() {
            return {
                form: {
                    username:      '13006638015',
                    password:      'Visitor!',
                    passwordAgain: 'Visitor!',
                    showWarning:   false,
                },
                rules: {
                    username: [
        ...
        methods: {
            ...
            validatePassWord(rule, value, callback) {
                if (!this.passwordType.test(value)) {
                    callback(
                        new Error('密码必须同时包含大写和小写字母, 数字或特殊字符')
                    );
                } else {
                    callback();
                }
